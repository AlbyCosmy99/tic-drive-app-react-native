import ServicesState from '@/app/types/reduxTypes/states/ServicesState';
...
const servicesSlice = createSlice({
   ...
   reducers: {
        setServicesChoosenByUsers(state: ServicesState, action: PayloadAction<number>) {
            state.servicesChoosenByUsers = [action.payload]
        },
        setServicesChoosenByWorkshops(state: ServicesState, action: PayloadAction<number>) {
            state.servicesChoosenByWorkshops = [action.payload]
            console.log(state.servicesChoosenByWorkshops)
        },
        addServiceChoosenByUsers(state: ServicesState, action: PayloadAction<number>) {
            if(!state.servicesChoosenByUsers.includes(action.payload)) {
                state.servicesChoosenByUsers.push(action.payload)
            }
        },
        addServiceChoosenByWorkshops(state: ServicesState, action: PayloadAction<number>) {
            if(!state.servicesChoosenByWorkshops.includes(action.payload)) {
                state.servicesChoosenByWorkshops.push(action.payload)
            }
            console.log(state.servicesChoosenByWorkshops)
        },
        removeServiceChoosenByUsers(state: ServicesState, action: PayloadAction<number>) {
        ...
        removeServiceChoosenByWorkshops(state: ServicesState, action: PayloadAction<number>) {
