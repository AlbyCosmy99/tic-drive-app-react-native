...
const AddNewPaymentMethodScreen = () => {
    ...
    return (
      ...
      >
        <SafeAreaViewLayout>
            <TicDriveNavbar isLoginAvailable={false} topContent={<Text className='font-semibold text-lg'>Enter payment details</Text>}/>
            <View className="my-4 flex-1">
                <TicDriveInputLabel value={cardHolder} onChange={(e) => setCardHolder(e)} label="Cardholder name *" placeholder="e.g. John A. Doe" />
                <TicDriveInputLabel value={cardNumber} onChange={(e) => setCardNumber(e)} label="Card number" placeholder="Card number" />
                <TicDriveInputLabel label="Expiration Date" placeholder="MM/YY" />
                <View className="relative">
                    <TicDriveInputLabel label="Security Code (CVV)" placeholder="e.g. 123" />
                    <Text className="absolute bottom-0 mx-2.5 text-tic text-sm">The 3-digit number on the back of your card.</Text>
                </View>
            </View>
            <HorizontalLine />
            <TicDriveButton disabled={!cardHolder} text="Add card" onClick={onConfirm}/>
        </SafeAreaViewLayout>
      </LinearGradient>
    )
}
