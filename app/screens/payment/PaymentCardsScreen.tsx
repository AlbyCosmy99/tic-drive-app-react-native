...
const PaymentCardsScreen = () => {
    ...
    useEffect(() => {
        if(!userPaymentInfo?.customPaymentTypes.find(type => type.id === userPaymentInfo?.choosenCard?.id) && !userPaymentInfo?.customPaymentTypes.find(type => type.id === userPaymentInfo?.choosenCard?.id) &&  !([-1,-2,-3].includes(userPaymentInfo?.choosenCard?.id!))) {
            setUserPaymentInfo({
                ...userPaymentInfo,
        ...
        <SafeAreaViewLayout>
            <View className="flex-1 mx-3.5">
                <TicDriveNavbar 
                    ...
                    rightContent={
                        <Pressable onPress={() => navigationPush(navigation, 'AddNewPaymentMethodScreen')}>
                ...
                <ScrollView showsVerticalScrollIndicator={false}>
                    ...
                    <Pressable key={paymentType.id} className="my-2" onPress={() => setUserPaymentInfo({
