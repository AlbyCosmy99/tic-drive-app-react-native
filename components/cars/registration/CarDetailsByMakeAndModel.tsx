...
const CarDetailsByMakeAndModel: React.FC<CarDetailsByMakeAndModelProps> = ({ carSelected }) => {
    ...
    useEffect(() => {
        if(setCarSelectedByMakeAndModel) {
            ...
            setCarSelectedByMakeAndModel({...carSelectedByMakeAndModel, ...carSelected});
        }
    }, []);
    ...
    return (
        <>
            ...
            />
            <TicDriveDropdown placeholder="Choose the car fuel type" searchPlaceholder="Search fuel type" title="Fuel" data={fuels.map((fuel,index) => ({id: index, value: fuel}))} value={{id: -1, value: carSelectedByMakeAndModel?.fuel!}} setValue={setCarFuelType}/>
            <TicDriveTextOrInput
                title="Mileage"
