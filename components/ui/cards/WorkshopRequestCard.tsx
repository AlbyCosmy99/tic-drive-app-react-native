...
import PendingIcon from '@/components/svgs/requestStatus/Pending';
import AcceptedIcon from '@/components/svgs/requestStatus/Check';
import {Colors} from '@/constants/Colors';
import CalendarIcon from '@/components/svgs/EventAvailable';
import CarRepairIcon from '@/components/svgs/servicesIcons/CarRepair';
import CreditCardIcon from '@/components/svgs/payment/CreditScore';
import CarIcon from '@/components/svgs/vehicles/Car1';
    ...
    <View className="border border-slate-200 rounded-xl mt-4">
        ...
        >
          ...
          >
            {status === 'pending' ? (
              ...
              <AcceptedIcon width={20} height={20} />
            )}
            <Text
              className="font-medium"
              style={status === 'pending' ? styles.pending : styles.accepted}
            >
              {status}
            </Text>
          </View>
      ...
      {status === 'accepted' && (
        <View className="flex flex-row justify-center items-center bg-slate-100 p-3 my-2">
          ...
          <Text className="text-2xl" style={styles.pinValue}>
            55794
          </Text>
        </View>
      )}
