...
const PaymentCard:React.FC<PaymentCardProps> = ({icon, paymentType, userName, id, optionsVisible = true}) => {
    ...
    useEffect(() => {
        ...
        setIsCustomCard(!!userPaymentInfo?.customPaymentTypes.find(type => type.id === id))
    }, [userPaymentInfo?.choosenCard?.id])

    const onDeleteCard = (id: number) => {
        setUserPaymentInfo({
            ...userPaymentInfo,
            customPaymentTypes: userPaymentInfo!.customPaymentTypes.filter(card => card.id !== id),
    ...
    return (
        <View className="flex flex-row items-center border border-grey-light p-3 rounded-lg">
            <View className={`border w-16 h-9 py-1 rounded-lg border-grey-light flex justify-center items-center ${isCustomCard && 'mb-6'}`}>
                {icon}
            </View>
            <View className="mx-3">
                <View className="mb-2">
